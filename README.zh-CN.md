...
Hubo 把这场小小的对决放进你的编程代理。

Hubo 取自**互搏**（*hùbó*）：彼此过招。不是两个代理客气地轮流发言，也不是评审者站在门口说一句“看起来不错”。双手过招，打磨每次改动。只有真正需要你拍板的问题，才会交到你手上。它们交换证据，直到代码本身——而不是任何一个代理的自尊心——胜出。

## 前后对比

没有 Hubo 时，一个代理会实现功能、跑测试、复查自己的推理，然后把结果交给你。作者、测试者和评审者共享同样的盲区。
...
| 手 | 职责 |
|---|---|
| **工作代理** | 理解任务、修改代码、运行测试，并以修复或基于证据的反驳回答每一条评审意见。 |
| **评审代理** | 保持独立且只读，检查实际 diff 和验证证据，并返回带编号、可执行的问题。 |
| **协调者** | 在二者之间传递消息，跨轮次保留未解决问题，并且只在真正满足退出条件时停止。 |

循环会在以下情况之一发生时结束：

- 评审代理放行代码，所有问题都已达成一致；或
- 两个代理发现了只有你才能决定的产品、需求或技术方向问题。

评审代理可以派出专家，工作代理也可以委派实现任务。但最上层的结构始终不变：一只手创造，另一只手检验创造的结果。
