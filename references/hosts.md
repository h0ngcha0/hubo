...
Select one adapter from tools exposed in the current session. Preserve Botong's role boundaries and numbered reconciliation protocol regardless of host.

## Codex

Select this adapter when `spawn_agent`, `followup_task`, and `wait_agent` are available.

| Botong action | Codex mapping |
| --- | --- |
...
| Botong action | Claude Code mapping |
| --- | --- |
...
- If the host cannot create two addressable direct children, resume the same child, or return child results to the coordinator, do not claim Botong convergence. Ask whether the user accepts a sequential single-agent implementation and self-review instead.
